...
|--------------------|------------------|----------------------------------------------------------|
| `v1.14.1`          | Immutable        | Pinpoint identification of a specific release.           |
| `v1`               | Floating (force) | "The latest 1.x" — used by GitHub Action consumers.      |
| (none for `v1.14`) | n/a              | We do not ship floating minor tags.                      |

`v1.14.1` is created on every release. `v1` is **only** advanced when the
...
```yaml
- uses: xantorres/repokernel/.github/actions/rk-validate@v1.14.1
  with:
    fail-on: P0,P1
    version: 1.14.1
```
