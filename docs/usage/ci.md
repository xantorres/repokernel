...
| `working-directory` | `.` | Directory containing `repokernel.config.yaml`. Set when RK governs a sub-tree. |
...
| Output | Description |
|---|---|
| `exit-code` | `0` on success or skip; `1` on findings breach. Use to gate downstream steps. |
...
| Validate fails to run | `2` | neutral exit `0`, stderr in summary, no comment, no artifact |
