...
| `version` | `latest` | npm version of `repokernel`. Pin to a specific release (e.g. `1.30.0`) for reproducible CI. |
...
```yaml
- uses: xantorres/repokernel/.github/actions/rk-validate@v1.30.0
  with:
    fail-on: P0,P1
    version: 1.30.0   # do not float
```
...
```bash
npm install -g repokernel@1.30.0
