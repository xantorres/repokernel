...
|---|---|---|
| `awaiting_review` | Sprint complete, run is waiting for a human review verdict | `rk review-verdict R-NNN accepted` then `rk run E-001 --resume RUN-NNN` |
| `awaiting_reviews` | Parallel wave complete, all sprints in the wave need review verdicts | Set a verdict for each review, then `rk run E-001 --resume RUN-NNN` |
| `limit_reached` | Run hit the `--limit N` cap | `rk run E-001 --resume RUN-NNN` to continue; raise or omit `--limit` next time |
...
| `epic_completed` | All sprints are shipped — the epic is done | Nothing to do. Archive or close the epic. |
