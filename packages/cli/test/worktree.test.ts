...
describe('worktree naming', () => {
  it('uses distinct branch namespaces for epic and sprint worktrees', () => {
    expect(worktreeBranch(eid('E-001'), CONFIG)).toBe('rk/epic/E-001');
    expect(sprintWorktreeBranch(eid('E-001'), sid('S-001'), CONFIG)).toBe('rk/sprint/E-001/S-001');
  });

  it('keeps sprint worktrees outside the epic worktree directory', () => {
    const path = sprintWorktreePath(eid('E-001'), sid('S-001'), CONFIG, '/tmp/my-repo');
    expect(path).toMatch(
  ...
  it('namespaces same-basename repos to distinct worktree roots', () => {
    ...
    const alice = worktreePath(eid('E-001'), sharedRoot, '/home/alice/myapp');
    const bob = worktreePath(eid('E-001'), sharedRoot, '/home/bob/myapp');
...
describe('worktree naming — branchPattern', () => {
  ...
  it('uses a {sprintId} branchPattern for sprints while keeping epic default safe', () => {
    ...
    expect(worktreeBranch(eid('E-001'), config)).toBe('rk/epic/E-001');
    expect(sprintWorktreeBranch(eid('E-001'), sid('S-003'), config)).toBe('wip/E-001/S-003');
  });
  ...
  it('supports explicit epic and sprint branch patterns', () => {
    ...
    expect(worktreeBranch(eid('E-001'), config)).toBe('feature/epic/E-001');
    expect(sprintWorktreeBranch(eid('E-001'), sid('S-003'), config)).toBe(
      'feature/sprint/E-001/S-003',
    );
  });
  ...
  it('still uses default scheme when branchPattern is unset', () => {
    expect(worktreeBranch(eid('E-007'), CONFIG)).toBe('rk/epic/E-007');
    expect(sprintWorktreeBranch(eid('E-007'), sid('S-002'), CONFIG)).toBe('rk/sprint/E-007/S-002');
  });

  it('rejects {ticket} token at render time (reserved for v1.14)', () => {
    const config = configWithPattern('feature/{ticket}-{epicId}');
    expect(() => worktreeBranch(eid('E-001'), config)).toThrow(RepoKernelError);
    expect(() => worktreeBranch(eid('E-001'), config)).toThrow(/v1\.14/);
  });
  ...
  it('rejects unknown tokens at render time', () => {
    ...
    expect(() => worktreeBranch(eid('E-001'), config)).toThrow(RepoKernelError);
    expect(() => worktreeBranch(eid('E-001'), config)).toThrow(/unknown token/);
  });
